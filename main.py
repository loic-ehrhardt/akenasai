...
class GradientWidget(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.background_texture.blit_buffer(
            buf, colorfmt='rgba', bufferfmt='ubyte')
        self.bind(pos=self.update_background_pos_size,
                  size=self.update_background_pos_size)
        with self.canvas.before:
            self.background = Rectangle(
                texture=self.background_texture, pos=self.pos, size=self.size)
        self.bind(col1=self.update_background_texture,
                  col2=self.update_background_texture)
        if 'col1' in kwargs:
...
class GradientBoxLayout(BoxLayout, GradientWidget):
    ...
    def spawn_settings(self):
        ...
        s.bind(on_close=self.on_settings_end)
        self.parent.add_widget(s)

    def on_settings_end(self, s):
        s.parent.remove_widget(s)

class AkenasaiApp(App):
    pass
...
class WifiManager:
    def __init__(self):
        ...
        self.wifi_service = self.activity.getSystemService(
            self.Context.WIFI_SERVICE)
...
if __name__=='__main__':
    ...
    Clock.schedule_interval(
        wifimanager.update, config.getint('Wifi','update_interval'))
    APP.run()
